...
def imprimir_cartilla_marcada(serie, diccionario_cartillas): 
  ...
  print("\n")
def generador_numero_aleatorio(list):   
  i=randint(1,75)
  if i not in list:
    list.append(i)
    ...
    list.append(i)
    print("Numero:"," ".join(str(list[-1])))
def cantidad_numeros_marcados(matriz):
  contador = 0
  for i in range(5):
    for j in range(5):
  ...
  for tope in range(len(lista)-1, 0, -1):
   for i in range(tope):
     if cantidad_numeros_marcados(lista[i][1]) > cantidad_numeros_marcados(lista[i+1][1]):
       ...
       lista[i] = lista[i+1]
       lista[i+1] = temp
def guardar_archivo(serie, diccionario_cartillas,archivo, numeros_generados, nombre_usuario, fecha):
  archivo.write(fecha +"\n")
